# 校园跑腿小程序 - 完整API接口文档

## 基础配置

- **基础URL**: `http://localhost:3000/api` (开发环境)
- **认证方式**: Bearer Token (JWT)
- **请求格式**: JSON
- **超时时间**: 10秒

## 状态码规范

### HTTP状态码
- `200`: 成功
- `201`: 创建成功
- `400`: 请求参数错误
- `401`: 未授权
- `403`: 无权限
- `404`: 资源不存在
- `500`: 服务器错误

### 业务状态码
- `code: 0`: 成功
- `code: 400`: 业务错误

## 一、认证相关接口 🔐

### 1.1 用户登录
**接口**: `POST /auth/login`

**用途**: 支持三种登录方式：微信登录、用户名密码登录、邮箱密码登录

**请求参数**:
```json
// 微信登录
{
  "code": "微信登录code"
}

// 用户名密码登录
{
  "username": "user123",
  "password": "password123"
}

// 邮箱密码登录
{
  "email": "user@example.com",
  "password": "password123"
}
```

**响应**:
```json
{
  "success": true,
  "code": 0,
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
  "user": {
    "id": 1,
    "username": "user123",
    "email": "user@example.com",
    "role": "student",
    "nickname": "小明",
    "avatar": "http://..."
  },
  "message": "登录成功"
}
```

### 1.2 用户注册
**接口**: `POST /auth/register`

**用途**: 使用用户名和密码注册新账号

**请求参数**:
```json
{
  "username": "user123",
  "password": "password123",
  "confirmPassword": "password123",
  "email": "user@example.com" // 可选
}
```

**响应**:
```json
{
  "success": true,
  "code": 0,
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
  "user": {
    "id": 1,
    "username": "user123",
    "email": "user@example.com",
    "role": "student"
  },
  "message": "注册成功"
}
```

**注意事项**:
- 用户名必须唯一
- 密码长度至少6位
- 如果提供confirmPassword，必须与password一致
- email字段可选

### 1.3 发送验证码
**接口**: `POST /auth/send-code`

**请求参数**:
```json
{
  "phone": "13800138000",
  "email": "user@example.com",
  "type": "login"
}
```

### 1.4 验证验证码
**接口**: `POST /auth/verify-code`

**请求参数**:
```json
{
  "phone": "13800138000",
  "code": "123456"
}
```

### 1.5 退出登录
**接口**: `POST /auth/logout`

**需要认证**: ✅

## 二、用户管理接口 👤

### 2.1 用户基础信息

#### 获取用户信息
**接口**: `GET /users/profile`

**需要认证**: ✅

**响应**:
```json
{
  "success": true,
  "code": 0,
  "data": {
    "id": 1,
    "username": "user123",
    "nickname": "小明",
    "avatar": "http://...",
    "phone": "13800138000",
    "email": "user@example.com"
  }
}
```

#### 更新用户信息
**接口**: `PUT /user/info`

**需要认证**: ✅

**请求参数**:
```json
{
  "nickname": "新昵称",
  "avatar": "http://...",
  "phone": "13800138000"
}
```

#### 获取头像
**接口**: `GET /user/avatar`

**需要认证**: ✅

#### 更新头像
**接口**: `POST /user/avatar`

**需要认证**: ✅

**请求参数**:
```json
{
  "avatar": "http://..."
}
```

### 2.2 实名认证

#### 提交实名认证
**接口**: `POST /user/certify`

**需要认证**: ✅

**请求参数**:
```json
{
  "realName": "张三",
  "idCard": "110101199001011234",
  "idCardFront": "http://...",
  "idCardBack": "http://..."
}
```

#### 获取认证状态
**接口**: `GET /user/certification/status`

**需要认证**: ✅

**响应**:
```json
{
  "success": true,
  "code": 0,
  "data": {
    "status": "approved",
    "realName": "张三"
  }
}
```

#### 获取认证信息
**接口**: `GET /user/certification/info`

**需要认证**: ✅

### 2.3 钱包功能

#### 获取钱包信息
**接口**: `GET /user/wallet`

**需要认证**: ✅

**响应**:
```json
{
  "success": true,
  "code": 0,
  "data": {
    "balance": 100.00,
    "frozen": 10.00,
    "total": 110.00
  }
}
```

#### 获取钱包明细
**接口**: `GET /user/wallet/details`

**需要认证**: ✅

**查询参数**:
- `page`: 页码
- `pageSize`: 每页数量
- `type`: 类型筛选

#### 提现
**接口**: `POST /user/wallet/withdraw`

**需要认证**: ✅

**请求参数**:
```json
{
  "amount": 50.00,
  "account": "支付宝账号"
}
```

#### 充值
**接口**: `POST /user/wallet/recharge`

**需要认证**: ✅

**请求参数**:
```json
{
  "amount": 100.00,
  "paymentMethod": "wechat"
}
```

### 2.4 收藏和历史

#### 获取收藏列表
**接口**: `GET /user/favorites`

**需要认证**: ✅

**查询参数**:
- `page`: 页码
- `pageSize`: 每页数量

#### 添加收藏
**接口**: `POST /user/favorites`

**需要认证**: ✅

**请求参数**:
```json
{
  "orderId": 123
}
```

#### 取消收藏
**接口**: `DELETE /user/favorites/:id`

**需要认证**: ✅

#### 获取历史记录
**接口**: `GET /user/history`

**需要认证**: ✅

#### 删除历史记录
**接口**: `DELETE /user/history/:id`

**需要认证**: ✅

## 三、订单核心接口 📦

### 3.1 订单CRUD操作

#### 获取订单列表（大厅）
**接口**: `GET /orders`

**需要认证**: ✅

**用途**: 获取订单大厅列表，默认只显示未完成的订单（pending和accepted状态）

**查询参数**:
- `page`: 页码 (默认1)
- `pageSize`: 每页数量 (默认10)
- `status`: 状态筛选 (默认: pending,accepted)
  - 可选值: pending/accepted/completed/cancelled
  - 支持多个状态，用逗号分隔，如: pending,accepted
- `type`: 类型筛选
- `keyword`: 关键词搜索

**响应**:
```json
{
  "success": true,
  "code": 0,
  "data": [
    {
      "id": 1,
      "user_id": 1,
      "title": "快递代取",
      "description": "帮忙取个快递",
      "type": 1,
      "price": 5.00,
      "status": "pending",
      "publisher_name": "小明",
      "publisher_avatar": "http://...",
      "created_at": "2024-01-01T00:00:00.000Z"
    }
  ]
}
```

**说明**:
- 订单大厅默认不显示已完成(completed)和已取消(cancelled)的订单
- 已完成的订单会保留在数据库中，可通过"我发布的订单"或"我接受的订单"查看
- 每个订单都绑定了发布者的user_id

#### 获取订单详情
**接口**: `GET /orders/:orderId`

**需要认证**: ✅

#### 创建新订单
**接口**: `POST /orders`

**需要认证**: ✅

**请求参数**:
```json
{
  "title": "快递代取",
  "description": "帮忙取个快递",
  "type": 1,
  "price": 5.00,
  "pickupLocation": "菜鸟驿站",
  "deliveryLocation": "宿舍楼下",
  "contactPhone": "13800138000",
  "images": ["http://..."]
}
```

#### 更新订单
**接口**: `PUT /orders/:orderId`

**需要认证**: ✅

#### 删除订单
**接口**: `DELETE /orders/:orderId`

**需要认证**: ✅

### 3.2 订单状态流转

#### 接单
**接口**: `POST /orders/:orderId/accept`

**需要认证**: ✅

#### 取消订单
**接口**: `POST /orders/:orderId/cancel`

**需要认证**: ✅

**请求参数**:
```json
{
  "reason": "取消原因"
}
```

#### 完成订单
**接口**: `POST /orders/:orderId/complete`

**需要认证**: ✅

#### 确认收货
**接口**: `POST /orders/:orderId/confirm`

**需要认证**: ✅

### 3.3 订单查询分类

#### 我发布的订单
**接口**: `GET /orders/my-publish`

**需要认证**: ✅

**查询参数**:
- `page`: 页码
- `pageSize`: 每页数量
- `status`: 状态筛选

#### 我接受的订单
**接口**: `GET /orders/my-accepted`

**需要认证**: ✅

#### 搜索订单
**接口**: `GET /orders/search`

**需要认证**: ✅

**查询参数**:
- `keyword`: 搜索关键词
- `page`: 页码
- `pageSize`: 每页数量

#### 热门订单
**接口**: `GET /orders/hot`

**需要认证**: ✅

#### 推荐订单
**接口**: `GET /orders/recommended`

**需要认证**: ✅

### 3.4 订单评价举报

#### 订单评价
**接口**: `POST /orders/:orderId/evaluate`

**需要认证**: ✅

**请求参数**:
```json
{
  "rating": 5,
  "comment": "服务很好",
  "tags": ["快速", "友好"]
}
```

#### 获取评价
**接口**: `GET /orders/:orderId/evaluations`

**需要认证**: ✅

#### 举报订单
**接口**: `POST /orders/:orderId/report`

**需要认证**: ✅

**请求参数**:
```json
{
  "reason": "违规内容",
  "description": "详细描述"
}
```

#### 分享订单
**接口**: `POST /orders/:orderId/share`

**需要认证**: ✅

## 四、通知系统接口 🔔

#### 获取通知列表
**接口**: `GET /notifications`

**需要认证**: ✅

**查询参数**:
- `page`: 页码
- `pageSize`: 每页数量
- `unreadOnly`: 只看未读 (true/false)

#### 未读通知数量
**接口**: `GET /notifications/unread-count`

**需要认证**: ✅

**响应**:
```json
{
  "success": true,
  "code": 0,
  "data": {
    "count": 5
  }
}
```

#### 标记已读
**接口**: `PUT /notifications/:notificationId/read`

**需要认证**: ✅

#### 全部标记已读
**接口**: `PUT /notifications/read-all`

**需要认证**: ✅

#### 删除通知
**接口**: `DELETE /notifications/:notificationId`

**需要认证**: ✅

## 五、系统通用接口 ⚙️

### 5.1 系统配置

#### 获取系统配置
**接口**: `GET /system/config`

**响应**:
```json
{
  "success": true,
  "code": 0,
  "data": {
    "appName": "校园跑腿",
    "version": "1.0.0",
    "apiBaseUrl": "http://localhost:3000/api",
    "uploadMaxSize": 10485760,
    "orderMinPrice": 1,
    "orderMaxPrice": 1000
  }
}
```

#### 获取服务类型列表
**接口**: `GET /system/service-types`

#### 获取版本信息
**接口**: `GET /system/version`

#### 检查更新
**接口**: `POST /system/check-update`

**请求参数**:
```json
{
  "version": "1.0.0"
}
```

### 5.2 文件上传

#### 上传单张图片
**接口**: `POST /upload/single`

**需要认证**: ✅

**请求格式**: `multipart/form-data`

**请求参数**:
- `image` (file, 必填): 图片文件
- `type` (string, 可选): 上传类型
  - `avatar`: 用户头像
  - `orders`: 订单图片（默认）

**响应**:
```json
{
  "success": true,
  "message": "上传成功",
  "data": {
    "url": "/uploads/avatars/1703001234567-123456789.jpg",
    "filename": "1703001234567-123456789.jpg",
    "size": 102400,
    "mimetype": "image/jpeg"
  }
}
```

**限制**:
- 文件大小: 最大 5MB
- 支持格式: jpeg, jpg, png, gif, webp

#### 上传多张图片
**接口**: `POST /upload/multiple`

**需要认证**: ✅

**请求格式**: `multipart/form-data`

**请求参数**:
- `images` (file[], 必填): 图片文件数组（最多9张）
- `type` (string, 可选): 上传类型，默认 `orders`

**响应**:
```json
{
  "success": true,
  "message": "上传成功",
  "data": {
    "images": [
      {
        "url": "/uploads/orders/1703001234567-123456789.jpg",
        "filename": "1703001234567-123456789.jpg",
        "size": 102400,
        "mimetype": "image/jpeg"
      }
    ],
    "count": 1
  }
}
```

#### 删除图片
**接口**: `DELETE /upload/delete`

**需要认证**: ✅

**请求参数**:
```json
{
  "filename": "1703001234567-123456789.jpg",
  "type": "avatar"
}
```

**响应**:
```json
{
  "success": true,
  "message": "删除成功"
}
```

#### 访问图片
**URL格式**: `/uploads/{type}/{filename}`

**示例**:
- 头像: `http://localhost:3000/uploads/avatars/1703001234567-123456789.jpg`
- 订单图片: `http://localhost:3000/uploads/orders/1703001234567-123456789.jpg`

**说明**:
- 图片通过静态文件服务直接访问
- 无需认证即可访问
- 数据库中存储相对路径，如 `/uploads/avatars/xxx.jpg`

### 5.3 位置服务

#### 获取位置信息
**接口**: `GET /system/location`

**查询参数**:
- `latitude`: 纬度
- `longitude`: 经度

#### 搜索地点
**接口**: `GET /system/search-location`

**查询参数**:
- `keyword`: 搜索关键词

#### 获取天气信息
**接口**: `GET /system/weather`

**查询参数**:
- `city`: 城市名称

### 5.4 内容管理

#### 获取公告列表
**接口**: `GET /system/announcements`

**查询参数**:
- `page`: 页码
- `pageSize`: 每页数量

#### 获取热门搜索
**接口**: `GET /system/hot-search`

**响应**:
```json
{
  "success": true,
  "code": 0,
  "data": ["快递代取", "食堂代购", "打印复印"]
}
```

#### 获取推荐关键词
**接口**: `GET /system/recommended-keywords`

#### 敏感词检查
**接口**: `POST /system/check-sensitive`

**需要认证**: ✅

**请求参数**:
```json
{
  "content": "要检查的内容"
}
```

### 5.5 用户反馈

#### 提交意见反馈
**接口**: `POST /system/feedback`

**需要认证**: ✅

**请求参数**:
```json
{
  "type": "bug",
  "content": "反馈内容",
  "contact": "联系方式",
  "images": ["http://..."]
}
```

#### 获取帮助信息
**接口**: `GET /system/help`

#### 获取关于我们
**接口**: `GET /system/about`

#### 获取隐私政策
**接口**: `GET /system/privacy`

#### 获取用户协议
**接口**: `GET /system/agreement`

## 分页参数说明

大部分列表接口支持以下分页参数：

- `page`: 页码 (默认1)
- `pageSize`: 每页数量 (默认10或20)
- `status`: 状态筛选
- `keyword`: 关键词搜索

## 错误处理

所有接口在发生错误时返回统一格式：

```json
{
  "success": false,
  "code": 400,
  "message": "错误信息描述"
}
```

## 认证说明

需要认证的接口需要在请求头中携带 Token：

```
Authorization: Bearer <your_token_here>
```

Token 在登录成功后获取，有效期根据配置而定（默认7天）。
